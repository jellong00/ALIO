...
st.caption("내가 근무하는 기관, 혹은 관심 있는 기관을 직접 찾아 전체·동일유형·동일부처와 비교해보는 페이지입니다.")
...
st.markdown("### 📌 B. 핵심 KPI")
KPI_GROUPS = {
    "기관·인력": ["임직원수", "여성직원비율"],
    "재정": ["총수입", "정부지원의존도"],
    "보수·복지": ["직원평균보수", "1인당복리후생비"],
    "채용·일가정": ["신규채용률", "여성육아휴직사용자수"],
}
...
# ---------------- C. 4중 비교표 ----------------
st.markdown("### 📊 C. 4중 비교표 (기관 vs 동일유형 vs 동일부처 vs 전체)")
...
show_lines = st.multiselect("표시할 선 (최대 4개)", ["선택기관", "동일유형 평균", "동일부처 평균", "전체 평균"],
                              default=["선택기관", "동일유형 평균", "동일부처 평균"], max_selections=4, key="p6_tslines")
...
# ---------------- 유사기관 ----------------
st.markdown("### 🧭 유사기관 찾기")
sim_scope = st.radio("검색 범위", ["전체 기관", "동일 기관유형", "동일 주무부처"], horizontal=True, key="p6_simscope")
...
if org_name in snap_all["기관명"].values and snap_all.shape[0] > 5:
    ...
    st.caption("⚠️ 유사기관 결과는 선택된 변수, 검색 범위, 거리계산 방식(표준화 유클리드 거리)에 따라 달라질 수 있습니다.")
else:
