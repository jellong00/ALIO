...
with cc2:
    ...
    use_dept = st.checkbox("주무부처 통제 (더미)", value=False, key="p8_dept")
...
cat3 = ["기관유형"] if use_org_type else []
cat4 = cat3 + (["주무부처"] if use_dept else [])

models_spec = [
    ("Model 1", iv_cols, [], False, False),
    ("Model 2", iv_cols + numeric_controls, [], False, False),
    ("Model 3", iv_cols + numeric_controls, cat3, False, False),
    ("Model 4", iv_cols + numeric_controls, cat4, False, False),
    ("Model 5", iv_cols + numeric_controls, cat4, False, use_year),
]
...
if coef_rows:
    ...
    with st.expander("💡 계량분석 포인트"):
        st.markdown("- 통제변수를 추가했을 때 계수의 크기·부호·유의성이 어떻게 바뀌는지 확인하세요.\n"
                     ...
                     "- **기관유형 통제**는 '공기업(시장형)/준정부기관' 같은 큰 범주 차이를 통제하고, "
                     "**주무부처 통제**는 같은 부처 산하 기관들이 공유하는 정책·예산 환경까지 통제합니다. "
                     "두 통제변수를 각각 추가했을 때 계수가 다르게 변한다면, 어느 수준의 이질성이 더 크게 작용하는지 짐작할 수 있습니다.")
