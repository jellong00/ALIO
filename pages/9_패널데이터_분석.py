...
if dv_col in df.columns and iv_col in df.columns:
    specs = [("Pooled OLS", False, False), ("기관 고정효과", True, False),
             ("연도 고정효과", False, True), ("기관+연도 고정효과", True, True)]
    rows = []
    tabs = st.tabs([s[0] for s in specs])
    for (name, efe, yfe), tab in zip(specs, tabs):
        with tab:
            res, data, X = run_ols(df, dv_col, [iv_col], entity_fe=efe, year_fe=yfe)
            if res is None:
    ...
    st.info(
        ...
        "두 모형의 계수가 크게 다르다면, 관측되지 않은 기관 고유 특성이 Pooled OLS 결과에 영향을 주고 있었을 가능성이 있습니다."
    )
else:
