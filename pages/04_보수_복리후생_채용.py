...
from utils.level_compare import render_four_level_panel

st.set_page_config(page_title="보수·복리후생·채용", layout="wide")
st.title("④ 보수 · 복리후생 · 채용")
st.caption("탭을 이동하며 보수, 임원, 복리후생, 채용 지표를 살펴보고, 선택 기관을 전체·동일유형·동일부처와 비교하는 페이지입니다.")
...
sel_org = st.selectbox("🏢 비교할 기관 선택 (모든 탭에 공통 적용, 미선택 가능)", ["(선택 안 함)"] + orgs, key="p4_selorg")
st.divider()
...
with tab1:
    ...
    if sel_org != "(선택 안 함)":
        st.markdown(f"#### 🏢 {sel_org} — 전체·동일유형·동일부처 비교")
        render_four_level_panel(st, df, col, get_label(var_key), get_unit(var_key), sel_org)
...
with tab2:
    ...
    if sel_org != "(선택 안 함)":
        st.markdown(f"#### 🏢 {sel_org} — 전체·동일유형·동일부처 비교")
        render_four_level_panel(st, df, col2, get_label(var_key2), get_unit(var_key2), sel_org)

    st.markdown("#### 기관장-직원 보수배율 Top / Bottom")
    st.caption("기관마다 가장 최근 연도 값 1개만 사용합니다 (동일 기관이 여러 연도로 중복 표시되지 않습니다).")
...
with tab3:
    ...
    if sel_org != "(선택 안 함)":
        st.markdown(f"#### 🏢 {sel_org} — 전체·동일유형·동일부처 비교")
        render_four_level_panel(st, df, col3, get_label(var_key3), get_unit(var_key3), sel_org)
...
with tab4:
    ...
    if sel_org != "(선택 안 함)":
        st.markdown(f"#### 🏢 {sel_org} — 전체·동일유형·동일부처 비교")
        render_four_level_panel(st, df, col4, get_label(var_key4), get_unit(var_key4), sel_org)
