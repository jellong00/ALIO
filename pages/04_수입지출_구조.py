...
st.caption("재정 규모가 보수·채용 같은 다른 부문과 어떻게 연결되는지 미리 살펴봅니다. 더 자유로운 조합은 ⑧⑨번 페이지에서 확인할 수 있습니다.")
...
for c, (xk, yk, label) in zip([cc1, cc2], cross_pairs):
    ...
    with c:
        if xcol in view.columns and ycol in view.columns:
            ...
            st.plotly_chart(fig_c, use_container_width=True, key=f"p4_cross_{xk}_{yk}")
            if sub.shape[0] > 2:
