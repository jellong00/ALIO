...
st.title("③ 재정구조 및 법인세")
st.caption("공공기관의 수입·지출 구조와 법인세 흐름을 살펴보는 페이지입니다.")
...
# ---------------- 수입구조 100% stacked bar ----------------
st.markdown("### 📊 기관유형별 수입 구조 (100% 기준)")
comp = df[["기관명", "기관유형", "연도", "총수입", "정부지원수입", "사업수입"]].dropna(subset=["총수입"]).copy()
...
# ---------------- 개별기관 수입구조 ----------------
st.markdown("### 🍩 개별 기관 수입구조 vs 동일유형 평균")
c1, c2 = st.columns(2)
with c1:
...
if sel_year is not None:
    ...
    if not row.empty:
        r = row.iloc[0]
        org_type = df[df["기관명"] == sel_org]["기관유형"].iloc[0]
        type_avg = grp.loc[org_type] if org_type in grp.index else None
        d1, d2 = st.columns(2)
        with d1:
        ...
        with d2:
            if type_avg is not None:
                st.plotly_chart(plot_donut(["정부지원수입", "사업수입", "기타수입"],
                                             ...
                                             title=f"{org_type} 평균"), use_container_width=True)
