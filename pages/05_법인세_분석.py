...
if tax_col in view.columns:
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(plot_histogram(view, tax_col, var_key=tax_var), use_container_width=True)
    with c2:
        st.plotly_chart(plot_boxplot(view, tax_col, var_key=tax_var), use_container_width=True)
