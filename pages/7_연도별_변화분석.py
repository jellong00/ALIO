...
st.markdown(f"### 📈 전체 {agg} 추이 (수준)")
overall = df.groupby("연도")[col].agg(agg_func).reset_index()
fig1 = px.line(overall, x="연도", y=col, markers=True, labels={col: f"{get_label(var_key)} ({get_unit(var_key)})"})
fig1.update_traces(line=dict(width=3, color="#4C78A8"), marker=dict(size=9))
fig1.update_layout(font=dict(size=16), height=460)
st.plotly_chart(fig1, use_container_width=True)

st.markdown(f"### 📊 기관유형별 {agg} 추이")
by_type = df.groupby(["기관유형", "연도"])[col].agg(agg_func).reset_index()
fig2 = px.line(by_type, x="연도", y=col, color="기관유형", markers=True,
                color_discrete_map=ORG_TYPE_COLORS, labels={col: f"{get_label(var_key)} ({get_unit(var_key)})"})
fig2.update_layout(font=dict(size=16), height=500)
