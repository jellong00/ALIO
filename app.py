...
with qc1:
    if q_org:
        ...
        st.info("💰 기관장 연봉은 직원 평균보수의 몇 배일까요? — [보수 및 복리후생] 페이지에서 확인해보세요.")
with qc2:
...
st.markdown("### 페이지 안내")
st.caption("페이지는 5개 섹션으로 묶여 있습니다. 모든 페이지를 매번 쓸 필요는 없고, 필요한 페이지만 골라 쓰는 '분석 모듈'이라고 보면 됩니다.")

st.markdown("""
**I. 데이터 이해**
1. 변수분포 및 기술통계 — 선택한 변수는 어떤 분포를 갖는가? (8개 부문 탭, 왜도·로그변환 포함, 모든 변수 분포의 출발점)
2. 기관유형별 비교 — 기관유형에 따라 평균·분포가 다른가? (효과크기 η²/Cohen's d 포함)
3. 주무부처별 비교 — 주무부처별 산하기관은 어떻게 다른가? (기관유형×주무부처 교차분석 포함)

**II. 공공기관 주요 지표**
4. 수입·지출 구조 — 기관별 재정규모와 수입구성은 어떻게 다른가? (정부지원 금액 vs 의존도 사분면 포함)
5. 법인세 분석 — 과세표준·결정세액은 어떻게 분포하고 연관되는가? (결정세액 0/양수 비율, 실효법인세율 포함)
6. 보수 및 복리후생 — 보수·임원보수·복리후생 구조는 어떻게 다른가? (일반 분포는 ①번 참고)
7. 채용 및 인력구성 — 채용규모와 채용률·성별구성은 어떻게 다른가? ([일·가정양립] 탭 포함)

**III. 기관 단위 분석**
8. 기관 프로필 — 특정 기관은 최신연도 기준 전체·유형·부처에서 어디쯤인가? (실제 순위·z-score 특징지표·유사기관 포함)
9. 기관별 구조와 변화 — 선택기관의 재정·인력·보수·법인세 구조는 시간에 따라 어떻게 달라졌는가? (수입 증감분해, 기관 내 변동성 포함)

**IV. 탐색적 계량분석**
10. 두 변수 관계분석 — 두 지표는 얼마나 함께 움직이는가? (이상치 민감도·부분상관 포함)
11. 집단별 관계 및 상관구조 — 전체 관계가 유형·부처 내부에서도 같은가? (부문간 관계 지도 포함)
12. 시간 변화 — 수준·변화율·순위·기관 간 격차(CV)는 시간에 따라 어떻게 달라졌는가?

**V. 심화 계량분석**
13. 다중회귀분석 — 통제 후에도 관계가 유지되는가? (표준화계수·계수변화율 포함)
