...
**III. 탐색적 계량분석**
8. 두 변수 관계분석 — 두 지표는 얼마나 함께 움직이는가? (이상치 민감도·부분상관 포함)
9. 집단별 관계 및 상관구조 — 전체 관계가 유형·부처 내부에서도 같은가? (부문간 관계 지도 포함)
10. 기관 프로필 — 특정 기관은 전체·유형·부처에서 어디쯤인가? (실제 순위·z-score 특징지표 포함)
11. 시간 변화 — 수준·변화율·순위·기관 간 격차(CV)는 시간에 따라 어떻게 달라졌는가?

**IV. 심화 계량분석**
12. 다중회귀분석 — 통제 후에도 관계가 유지되는가? (표준화계수·계수변화율 포함)
13. 패널데이터 분석 — 기관 간 차이와 기관 내부 변화는 어떻게 다른가?
""")
